...
(
	...
	echo "^p(+10)^fg(#D8DEE9)^fn($icons)  ^fn($font1) WM ^p(+74)^fn($icons)^fn($font1) $de"
	echo "^p(+13)^fg(#D8DEE9)^fn($icons) ^fn($font1) LAYOUT ^p(+48)^fn($icons)^fn($font1) $Layout"
	echo "^p(+11)^fg(#D8DEE9)^fn($icons)  ^fn($font1) ^p(+6)SHELL ^p(+52)^fn($icons)^fn($font1) $Shell"
	echo ""
	echo "^fg(#4C566A)^p(+10)----------------------------------------"
	...
	echo ""
) | dzen2 -p -x 50 -y 745 -w 297 -bg $BG -fg $FG -l 49 -sa l -ta c -e 'onstart=uncollapse;button1=exit;button4=exit' -fn "Fantasque Sans Mono" &
transset-df .8

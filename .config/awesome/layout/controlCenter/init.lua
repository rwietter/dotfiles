...
awful.screen.connect_for_each_screen(function(s)
	...
	local slide_end = gears.timer({
		single_shot = true,
		timeout = 0.33 + 0.08,
		callback = function() control_c.visible = false end
	})
	...
	cc_toggle = function(screen)
		...
		control_c.x = screen.geometry.x + dpi(beautiful.useless_gap)
	...
	function show_extra_control_stuff(input)
		...
		slide_right.target = s.geometry.height -
				(control_c.height + beautiful.useless_gap * 2)
	end
	...
	control_c:setup {
		{
			{
				{ profile, nil, sessions, layout = wibox.layout.align.horizontal },
				sliders,
				song,
				services,

...
local function create_button(shape, color, command, c)
	...
	local w = wibox.widget {
		...
		shape = shape or
				helpers.prrect(beautiful.rounded, true, true, false, true),
		forced_width = dpi(15),
		forced_height = dpi(15)
	}

	-- hover effect
	w:connect_signal('mouse::enter', function() w.bg = beautiful.accent end)

	w:connect_signal('mouse::leave',
		function() w.bg = beautiful.fg_color .. "4D" end)

	-- press effect
	w:connect_signal('button::press',
		function() w.bg = beautiful.fg_color .. "99" end)

	w:connect_signal('button::release', function() w.bg = beautiful.accent end)
			...
			{
				{
					nil,
					{
						...
						create_button(gears.shape.circle, beautiful.fg_color,
							function() c:kill() end, c),

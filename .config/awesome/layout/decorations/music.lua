...
local song_artist = wibox.widget {
	...
	markup = helpers.colorize_text("Unknown",
		beautiful.ext_light_fg or beautiful.fg_color),
...
local song_name = wibox.widget {
	widget = wibox.widget.textbox,
	markup = helpers.colorize_text("None",
		beautiful.ext_light_fg or beautiful.fg_color),
...
local next_button = wibox.widget {
	widget = wibox.widget.textbox,
	markup = helpers.colorize_text("", beautiful.ext_light_fg or
...
playerctl:connect_signal("metadata",
	function(_, title, artist, album_path, _, __, ___)
		if title == "" then title = "None" end
		if artist == "" then artist = "Unknown" end
		if album_path == "" then album_path = beautiful.images.album_art end

		album_art:set_image(gears.surface.load_uncached(album_path))
		song_name:set_markup_silently(helpers.colorize_text(title,
			beautiful.ext_light_fg or
			beautiful.fg_color))
		song_artist:set_markup_silently(helpers.colorize_text(artist,
			beautiful.ext_light_fg or
			beautiful.fg_color))
	end)
...
playerctl:connect_signal("position",
	function(_, current_pos, total_pos, player_name)
...
prev_button:connect_signal("mouse::leave", function()
	...
	prev_button.markup =
	helpers.colorize_text("", beautiful.fg_color .. "99")
end)
...
local music_init = function(c)
	...
	awful.titlebar(c, {
		...
		{
			{
				{ album_art, layout = wibox.layout.fixed.horizontal },
				{
					nil,
					{
						song_name,
